...
# delete all previously uploaded jpegs
    filelist = [ f for f in os.listdir("/home/daisyz/tf_files/upload_pic/") if f.endswith in (".jpg", ".jpeg") ]
    for f in filelist:
    	os.remove(f)
#-------------DAISY EDITS----------------------------------------
...
#--------------DAISY EDITS--------------------------------------------
	...
	WHERE products.sale_status IN ('completed', 'active') and product_templates.id={}
	GROUP BY name, sku, product_templates.id
	HAVING count(product_templates.id)>5
	ORDER BY count(product_templates.id) DESC LIMIT 5;
	    """.format(identifier.astype(str))
